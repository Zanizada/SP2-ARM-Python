...
```mermaid
flowchart TD
A[Início do programa] --> B[Importa as bibliotecas]
...
CCA([Carregador]) --> CCB[(
        ...
        protocolo
        veiculo)]
CDA([SessaoRecarga]) --> CDB[(
    carregador
    potencia_recebida_kw
    duracao_horas
    tarifa_kwh
    energia_entregue
    valor)]
CEA([ChargeGridIntelligence]) --> CEB[(
    limite_predio_kw
    consumo_predio_kw
    tarifa_kwh
    carregadores
...
subgraph Funções
CBAA([Veiculos]) --> CBAB("energia_necessaria()")
CBAA --> CBAC("prioridade_ia()")
CCAA([Carregador]) --> CCAB("protocolo_normalizado()")
CDAA([SessaoRecarga]) --> CDAB("__post_init__()")
CEAA([ChargeGridIntelligence]) --> CEAB("__init__()")
CEAA --> CEAC("adicionar_carregador()")
CEAA --> CEAD("potencia_disponivel()")
CEAA --> CEAE("autenticar_pagamento()")
CEAA --> CEAF("balancear_demanda()")
CEAA --> CEAG("resumo_financeiro()")
subgraph Função Principal
CFAA("main()")
end
end

B --> |from\ndataclass| BA[dataclass, field] --> C
B --> |from\nenum| BB[Enum] --> C
B --> |from\ndatetime| BC[datetime] --> C
B --> BD[random] --> C
...
C --> CB([Veiculos]) --> D
C --> CC([Carregador]) --> D
C --> CD([SessaoRecarga]) --> D
C --> CE([ChargeGridIntelligence]) --> D

D(Executa o main)

D ---> E[Simulações] --> F[Fim do código]
